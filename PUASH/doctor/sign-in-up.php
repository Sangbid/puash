...
$sts=mysqli_query($con,"SELECT * FROM doctors WHERE docEmail='".$_POST['email']."' and password='$password'and status='inactive'"); // ".$_POST['password']."
$count2=mysqli_fetch_array($sts);
if($count2>0){
  $_SESSION['errmsg']="Your account is not activated";
}
else{
  $ret=mysqli_query($con,"SELECT * FROM doctors WHERE docEmail='".$_POST['email']."' and password='$password'"); // ".$_POST['password']."
